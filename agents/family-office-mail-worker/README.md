...
- IO pre-market: `morning-briefing` -> `portfolio-review` (light) -> `family-email-formatting` (`brief` mode)
- IO post-close: `portfolio-review` -> `tax-loss-harvesting` (conditional) -> `rebalance` (conditional) -> `family-email-formatting` (`brief` mode)
- COS weekly priorities: `weekly-review` -> `task-management` (read-only summary) -> `family-email-formatting` (`brief` mode) -> `search-strategy`/`search` (conditional)

## Inbound reply composition
- Inbound family replies use `family-email-formatting` in `reply` mode.
- Scheduled briefs stay report-style with executive summary, deep dive, and provenance.
- Inbound replies should read like natural email: direct answer first, then explanatory reasoning in prose, with tables or charts only when they materially improve clarity.

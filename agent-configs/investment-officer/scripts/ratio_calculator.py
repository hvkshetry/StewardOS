#!/usr/bin/env python3
"""
Financial Ratio Calculator — StewardOS Investment Officer
...
MCP Data Sources (feed output from these tools as input):
  - sec-edgar.sec_edgar_financial → income_statement, balance_sheet, cash_flow fields
  - finance-graph.upsert_statement_line_items (read-back) → historical financial data
  - market-intel-direct.get_market_snapshot → market_data context (market cap, price)
  - ghostfolio.portfolio(operation="summary") → portfolio-level context

Complements skills: client-report, illiquid-valuation

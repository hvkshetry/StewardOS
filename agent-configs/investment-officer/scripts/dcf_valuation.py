#!/usr/bin/env python3
"""
DCF Valuation Model — StewardOS Investment Officer
...
MCP Data Sources (feed output from these tools as input):
  - sec-edgar.sec_edgar_financial → historical revenue, margins, capex
  - market-intel-direct.get_fred_series → risk-free rate
  - market-intel-direct.get_market_snapshot → beta for WACC
  - finance-graph.list_valuation_observations → valuation history context

Complements skills: dcf-model (offline sensitivity calculator)
...
class DCFModel:
    ...
    def calculate_wacc(self) -> float:
        ...
        weight_sum = debt_weight + equity_weight
        if weight_sum > 0 and abs(weight_sum - 1.0) > 1e-6:
            debt_weight = debt_weight / weight_sum
            equity_weight = equity_weight / weight_sum

#!/usr/bin/env python3
"""
Budget Variance Analyzer — StewardOS Household Comptroller
...
Source: alirezarezvani/claude-skills (MIT license)
Adapted for StewardOS agent invocation.

MCP Data Sources (feed output from these tools as input):
  - actual-budget.analytics(operation="spending_by_category") → actuals
  - actual-budget.budget → budget targets by category
  - actual-budget.analytics(operation="monthly_summary") → prior periods

Complements skills: budget-review (materiality thresholds)

Usage:
    python budget_variance_analyzer.py budget_data.json
    python budget_variance_analyzer.py budget_data.json --format json
    python budget_variance_analyzer.py budget_data.json --threshold-pct 10 --threshold-amt 100
"""
...
class BudgetVarianceAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            threshold_amt: Materiality threshold as dollar amount (default $100)
        """
    ...
    def department_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for v in self.variances:
            ...
            elif v["favorability"] == "Unfavorable":
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=100.0,
        help="Materiality threshold dollar amount (default: 100)",
    )

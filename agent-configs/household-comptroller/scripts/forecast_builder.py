#!/usr/bin/env python3
"""
Forecast Builder — StewardOS Household Comptroller

Driver-based cash flow forecasting with 13-week rolling projection,
...
MCP Data Sources (feed output from these tools as input):
  - actual-budget.analytics(operation="monthly_summary") → historical periods
  - actual-budget.analytics(operation="balance_history") → opening balances
  - actual-budget.schedule → recurring inflows/outflows
  - finance-graph.list_liabilities → liability payment schedule

Complements skills: cash-forecast (driver-based projection logic)
